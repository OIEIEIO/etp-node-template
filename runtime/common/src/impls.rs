//! Auxillary struct/enums for DNA runtime.
...
DNA_support::impl_account_data! {
	struct AccountData<Balance>
	for
		EtpInstance,
		DnaInstance
	where
		Balance = DNA_primitives::Balance
	{
		// other data
	}
}
...
where
	R: DNA_balances::Trait<EtpInstance> + pallet_authorship::Trait,
	<R as frame_system::Trait>::AccountId: From<DNA_primitives::AccountId>,
	<R as frame_system::Trait>::AccountId: Into<DNA_primitives::AccountId>,
	<R as frame_system::Trait>::Event: From<
		DNA_balances::RawEvent<
			<R as frame_system::Trait>::AccountId,
			<R as DNA_balances::Trait<EtpInstance>>::Balance,
			EtpInstance,
		>,
	>,
{
	fn on_nonzero_unbalanced(amount: NegativeImbalance<R>) {
		...
		<DNA_balances::Module<R, EtpInstance>>::resolve_creating(
			&<pallet_authorship::Module<R>>::author(),
			amount,
		);
		<frame_system::Module<R>>::deposit_event(
			<DNA_balances::RawEvent<_, _, EtpInstance>>::Deposit(author, numeric_amount),
		);
	}
}

pub mod support_DNA_in_the_future {
	...
	where
		R: DNA_balances::Trait<EtpInstance>,
		R::Balance: Into<u128>,
	{
		fn factor() -> u128 {
			let issuance: u128 =
				<DNA_balances::Module<R, EtpInstance>>::total_issuance().into();
