//! The dna-network runtime. This can be compiled with `#[no_std]`, ready for Wasm.
...
// --- DNA ---
#[cfg(feature = "std")]
pub use DNA_claims::ClaimsList;
#[cfg(feature = "std")]
pub use DNA_eth_relay::DagsMerkleRootsLoader;
#[cfg(feature = "std")]
pub use DNA_staking::{Forcing, StakerStatus};
...
// --- DNA ---
use DNA_balances_rpc_runtime_api::RuntimeDispatchInfo as BalancesRuntimeDispatchInfo;
use DNA_eth_offchain::crypto::AuthorityId as EthOffchainId;
use DNA_eth_relay::EthNetworkType;
use DNA_primitives::*;
use DNA_runtime_common::*;
use DNA_staking::EraIndex;
use DNA_staking_rpc_runtime_api::RuntimeDispatchInfo as StakingRuntimeDispatchInfo;
...
/// Runtime version (dna-network).
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("dna-network"),
	impl_name: create_runtime_str!("dna-network"),
	authoetp_version: 0,
	spec_version: 2,
	impl_version: 0,
...
impl pallet_session::Trait for Runtime {
	...
	type ValidatorIdOf = DNA_staking::StashOf<Self>;
...
impl pallet_grandpa::Trait for Runtime {
	...
	type HandleEquivocation = pallet_grandpa::EquivocationHandler<
		...
		DNA_primitives::fisherman::FishermanAppCrypto,
		Runtime,
		Offences,
	>;
}
...
parameter_types! {
	// Minimum 100 bytes/ETP deposited (1 MILLI/byte)
...
parameter_types! {
	pub const EtpExistentialDeposit: Balance = 100 * MILLI;
	pub const DnaExistentialDeposit: Balance = 10 * MICRO;
}
impl DNA_balances::Trait<EtpInstance> for Runtime {
	type Balance = Balance;
	type DustRemoval = ();
	type Event = Event;
	type ExistentialDeposit = EtpExistentialDeposit;
	type BalanceInfo = AccountData<Balance>;
	type AccountStore = System;
	type DustCollector = (Dna,);
}
impl DNA_balances::Trait<DnaInstance> for Runtime {
	type Balance = Balance;
	type DustRemoval = ();
	type Event = Event;
	type ExistentialDeposit = DnaExistentialDeposit;
	type BalanceInfo = AccountData<Balance>;
	type AccountStore = System;
	type DustCollector = (Etp,);
}
...
impl DNA_staking::Trait for Runtime {
	type Event = Event;
...
impl DNA_elections_phragmen::Trait for Runtime {
	type Event = Event;
	type ModuleId = ElectionsPhragmenModuleId;
	type Currency = Etp;
	type ChangeMembers = Council;
	type InitializeMembers = Council;
	type CurrencyToVote = support_DNA_in_the_future::CurrencyToVoteHandler<Self>;
...
impl DNA_treasury::Trait for Runtime {
	...
	type Event = Event;
	type EtpProposalRejection = Treasury;
	type DnaProposalRejection = Treasury;
...
parameter_types! {
	...
	pub const Prefix: &'static [u8] = b"Pay ETPs to the dna-network account:";
}
impl DNA_claims::Trait for Runtime {
	type Event = Event;
...
impl DNA_eth_backing::Trait for Runtime {
	type ModuleId = EthBackingModuleId;
	type Event = Event;
	type DetermineAccountId = DNA_eth_backing::AccountIdDeterminator<Runtime>;
...
impl DNA_eth_relay::Trait for Runtime {
...
impl DNA_eth_offchain::Trait for Runtime {
	type AuthorityId = EthOffchainId;
	type FetchInterval = FetchInterval;
}

impl DNA_header_mmr::Trait for Runtime {}
...
construct_runtime!(
	...
	where
		Block = Block,
		NodeBlock = DNA_primitives::Block,
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		// --- DNA ---
		// Basic stuff; balances is uncallable initially.
		Balances: DNA_balances::<Instance0>::{Module, Call, Storage, Config<T>, Event<T>},
		Dna: DNA_balances::<Instance1>::{Module, Call, Storage, Config<T>, Event<T>},

		// Consensus support.
		Staking: DNA_staking::{Module, Call, Storage, Config<T>, Event<T>, ValidateUnsigned},

		// Governance stuff; uncallable initially.
		ElectionsPhragmen: DNA_elections_phragmen::{Module, Call, Storage, Config<T>, Event<T>},

		// Claims. Usable initially.
		Claims: DNA_claims::{Module, Call, Storage, Config, Event<T>, ValidateUnsigned},

		EthBacking: DNA_eth_backing::{Module, Call, Storage, Config<T>, Event<T>},
		EthRelay: DNA_eth_relay::{Module, Call, Storage, Config<T>, Event<T>},
		EthOffchain: DNA_eth_offchain::{Module, Call},

		HeaderMMR: DNA_header_mmr::{Module, Call, Storage},
...
pub type SignedExtra = (
	...
	DNA_eth_relay::CheckEthRelayHeaderHash<Runtime>,
);
...
impl_runtime_apis! {
	...
	impl DNA_balances_rpc_runtime_api::BalancesApi<Block, AccountId, Balance> for Runtime {
		fn usable_balance(
			instance: u8,
			account: AccountId
	...
	impl DNA_staking_rpc_runtime_api::StakingApi<Block, AccountId, Power> for Runtime {
	...
	// 		steps: Vec<u32>,
	// 		repeat: u32,
	// 	) -> Result<Vec<frame_benchmarking::BenchmarkResults>, RuntimeStetp> {
