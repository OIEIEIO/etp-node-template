//! DNA service. Specialized wrapper over substrate service.
...
// --- DNA ---
pub use chain_spec::dna-networkChainSpec;
pub use dna-network_runtime;
pub use DNA_primitives::Block;
...
// --- DNA ---
use DNA_primitives::{AccountId, Balance, Nonce, Power};

native_executor_instance!(
	pub dna-networkExecutor,
	dna-network_runtime::api::dispatch,
	dna-network_runtime::native_version,
	// TODO: benchmarking
	// frame_benchmarking::benchmarking::HostFunctions,
);

/// A set of APIs that DNA-like runtimes must implement.
pub trait RuntimeApiCollection<Extrinsic: 'static + Send + Sync + codec::Codec>:
	...
	+ DNA_balances_rpc_runtime_api::BalancesApi<Block, AccountId, Balance>
	+ DNA_staking_rpc_runtime_api::StakingApi<Block, AccountId, Power>
...
pub trait IdentifyVariant {
	/// Returns if this is a configuration for the `dna-network` network.
	fn is_dna-network(&self) -> bool;
}

impl IdentifyVariant for Box<dyn ChainSpec> {
	fn is_dna-network(&self) -> bool {
		self.id().starts_with("dna-network")
	}
}

// If we're using prometheus, use a registry with a prefix of `DNA`.
fn set_prometheus_registry(config: &mut Configuration) -> Result<(), ServiceError> {
	if let Some(PrometheusConfig { registry, .. }) = config.prometheus_config.as_mut() {
		*registry = Registry::new_custom(Some("DNA".into()), None)?;
	}
...
macro_rules! new_full_start {
	($config:expr, $runtime:ty, $executor:ty) => {{
		...
		let builder = sc_service::ServiceBuilder::new_full::<Block, $runtime, $executor>($config)?
			...
			.with_rpc_extensions(|builder| -> Result<DNA_rpc::RpcExtension, _> {
...
macro_rules! new_full {
	(
		...
		let (role, is_authority, force_authoetp, name, disable_grandpa) = (
			$config.role.clone(),
			$config.role.is_authority(),
			$config.force_authoetp,
	...
	($config:expr, $runtime:ty, $dispatch:ty) => {{
		...
		ServiceBuilder::new_light::<Block, $runtime, $dispatch>($config)?
			...
			.with_rpc_extensions(|builder| -> Result<DNA_rpc::RpcExtension, _> {
				...
				let light_deps = DNA_rpc::LightDeps {
					remote_blockchain,
					fetcher,
					client: builder.client().clone(),
					pool: builder.pool(),
				...
				Ok(DNA_rpc::create_light(light_deps))
			})?
			.build()
		}};
}
...
/// Create a new dna-network service for a full node.
#[cfg(feature = "full-node")]
pub fn dna-network_new_full(mut config: Configuration) -> Result<impl AbstractService, ServiceError> {
	Ok(new_full!(config, dna-network_runtime::RuntimeApi, dna-networkExecutor))
}

/// Create a new dna-network service for a light client.
pub fn dna-network_new_light(
	mut config: Configuration,
) -> Result<
	impl AbstractService<
		Block = Block,
		RuntimeApi = dna-network_runtime::RuntimeApi,
		Backend = TLightBackend<Block>,
		SelectChain = LongestChain<TLightBackend<Block>, Block>,
		CallExecutor = TLightCallExecutor<Block, dna-networkExecutor>,
	>,
	ServiceError,
> {
	new_light!(config, dna-network_runtime::RuntimeApi, dna-networkExecutor)
}
